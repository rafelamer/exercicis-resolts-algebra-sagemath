...
class AsymptoteMagic(Magics):
    ...
    @argument(
        ...
        choices=["png", "xpm", "xbm", "pbm", "svg", "pdf"],
...
def run_asy_file(asy_file, img_file=None, fmt="png"):
    ...
    if fmt == "pdf":
        ...
        svg_proc = subprocess.Popen(["pdf2svg",img_file,svg_file],
                                stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE)
        svg_ret_code = svg_proc.wait()
        if svg_ret_code != 0:
            raise AsymptoteException(svg_proc.stderr.read())
        svg_stdout = asy_proc.stdout.read()

        return SVG(filename=svg_file), svg_stdout
