               ...
               real scaled=1.0)
{
  if(dot(u,(0,1)) == 0)
  {
  	HiperbolaSimple(o,a2,b2,x,y,xdir=true,scaled=scaled);
  	return;
  }
  if(dot(u,(1,0)) == 0)
  {
  	HiperbolaSimple(o,a2,b2,x,y,xdir=false,scaled=scaled);
  	return;
  }
  pen lx;
  lx = green+linewidth(scaled*0.4mm);
...
picture VectorField(pair f(real,real),
										pair a,
										pair b,
                    int nx=nmesh,
										int ny=nx,
										pen p=currentpen,
										arrowbar arrow=Arrow(1))
{
  picture pic;
  real dx=(b.x-a.x)/nx;
  real dy=(b.y-a.y)/ny;
